...
from model.oim import OIMLoss
from model.resnet import build_resnet
from model.mobilnet import build_mobilenet
from model.convnext import build_convnext


def _build_backbone(backbone_name, pretrained=True):
    """Dispatch to the correct backbone builder based on name."""
    if backbone_name in ("resnet50", "resnet34", "resnet101"):
        return build_resnet(name=backbone_name, pretrained=pretrained)
    elif backbone_name == "mobilenet_v3_large":
        return build_mobilenet(name=backbone_name, pretrained=pretrained)
    elif backbone_name == "convnext_small":
        return build_convnext(name=backbone_name, pretrained=pretrained)
    else:
        raise ValueError(f"Unknown backbone: {backbone_name}")


class SeqNet(nn.Module):
    def __init__(self, cfg, backbone_name="resnet50"):
        super(SeqNet, self).__init__()

        backbone, box_head = _build_backbone(backbone_name, pretrained=True)

        # Derive channel dimensions from the backbone/head
        backbone_channels = box_head.out_channels[0]  # e.g. 1024 (ResNet), 112 (MNv3-L), 48 (MNv3-S)
        head_channels = box_head.out_channels[1]       # e.g. 2048 (ResNet), 960 (MNv3-L), 576 (MNv3-S)
        ...
        pre_nms_top_n = dict(
            training=cfg.MODEL.RPN.PRE_NMS_TOPN_TRAIN, testing=cfg.MODEL.RPN.PRE_NMS_TOPN_TEST # 12000 - 6000
        )
        post_nms_top_n = dict(
            training=cfg.MODEL.RPN.POST_NMS_TOPN_TRAIN, testing=cfg.MODEL.RPN.POST_NMS_TOPN_TEST # 2000 - 300
        )
        ...
        faster_rcnn_predictor = FastRCNNPredictor(head_channels, 2)
        reid_head = deepcopy(box_head) # re-use the same architecture as box_head for the re-ID head
        box_roi_pool = MultiScaleRoIAlign(
            featmap_names=["feat_res4"], output_size=14, sampling_ratio=2
        )
        box_predictor = BBoxRegressor(head_channels, num_classes=2, bn_neck=cfg.MODEL.ROI_HEAD.BN_NECK)
        roi_heads = SeqRoIHeads(
            # OIM
            num_pids=cfg.MODEL.LOSS.LUT_SIZE, # 482 labeled training IDs in PRW
            num_cq_size=cfg.MODEL.LOSS.CQ_SIZE, # 500 for PRW
            oim_momentum=cfg.MODEL.LOSS.OIM_MOMENTUM, # 0.5
            oim_scalar=cfg.MODEL.LOSS.OIM_SCALAR, # 30.0
            # SeqNet
            faster_rcnn_predictor=faster_rcnn_predictor,
            reid_head=reid_head,
            # Channel dims for NormAwareEmbedding
            embedding_in_channels=[backbone_channels, head_channels],
            # parent class
        ...
        # here we apply the same image transformations as Faster R-CNN's GeneralizedRCNNTransform, which resizes and normalizes the input images.
        # Therefore there is no need to add additional resizing or normalization in the dataset transforms. The model's inference() method will pass the query images through this same transform, ensuring consistent preprocessing for both gallery and query inputs.
        transform = GeneralizedRCNNTransform(
            min_size=cfg.INPUT.MIN_SIZE, # 900
            max_size=cfg.INPUT.MAX_SIZE, # 1500
        ...
        # In total we have 5 main losses: L_reg1, L_cls1, L_reg2, L_cls2, L_reid. The first two are from RPN, the next two are from the box head, and the last one is from the re-ID head.
        # loss weights
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        # apply the same transformations as in training (resize + normalization)
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        reid_head,
        embedding_in_channels=None,
        *args,
        **kwargs
    ):
        super(SeqRoIHeads, self).__init__(*args, **kwargs)
        if embedding_in_channels is None:
            embedding_in_channels = [1024, 2048]
        self.embedding_head = NormAwareEmbedding(in_channels=embedding_in_channels)
