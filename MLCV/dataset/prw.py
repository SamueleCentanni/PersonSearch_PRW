...
class PRW(BaseDataset):
    def __init__(self, root, transforms, split, image_names=None, query_annotations=None):
        self.name = "PRW"
        self.img_prefix = osp.join(root, "frames")
        self.image_names = image_names
        self.query_annotations = query_annotations
        super(PRW, self).__init__(root, transforms, split)

    def _get_cam_id(self, img_name):
        """Extract camera ID from the image name."""
    ...
    def _load_split_img_names(self):
        ...
        assert self.split in ("train", "gallery", "val_gallery")
        if self.split in ("train", "val_gallery"):
    ...
    def _load_annotations(self):
        """
        Load the annotations for the specific split.
        """
        
        if self.split == "query":
            return self._load_queries()

        if self.split == "val_query":
            if self.query_annotations is None:
                raise ValueError("query_annotations must be provided for split='val_query'")
            return self.query_annotations
        ...
        for img_name in imgs:
            ...
            rois = anno[box_key][:, 1:] # region of interests, i.e., bounding boxes in (x1, y1, w, h) format
            ids = anno[box_key][:, 0]
